...
export const PortalHome = () => {
  const nextBooking = mockBookings[0];
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <Card className="border-2 border-primary/20 shadow-lg shadow-primary/5">
          <CardContent className="p-6">
            <div className="flex justify-between items-start">
              <div>
                <Badge variant="outline" className="text-primary border-primary/30 mb-2">Confirmed</Badge>
                <h3 className="text-xl font-bold leading-tight">{nextBooking.service}</h3>
              ...
              <div className="text-right">
                <p className="text-sm font-bold text-primary">{nextBooking.time}</p>
                <p className="text-xs text-muted-foreground">{nextBooking.date}</p>
              </div>
            </div>
            <div className="mt-8 flex items-center justify-between gap-4 pt-6 border-t">
               <div className="flex items-center gap-3">
                  <div className="h-8 w-8 rounded-full bg-muted flex items-center justify-center text-xs font-bold">
                    {nextBooking.staff.charAt(0)}
                  </div>
                  <div className="text-xs">
                    <p className="text-muted-foreground">Staff Member</p>
                    <p className="font-bold">{nextBooking.staff}</p>
                  </div>
               </div>
      ...
      <div className="grid grid-cols-1 gap-3">
        <Button className="h-16 justify-between px-6 rounded-2xl text-lg font-bold shadow-xl shadow-primary/20 group" size="lg">
          Book New Service
          <ArrowRight className="h-5 w-5 group-hover:translate-x-1 transition-transform" />
        </Button>
        <div className="grid grid-cols-2 gap-3">
          <Button variant="outline" className="h-20 flex-col gap-2 rounded-2xl border-2">
      ...
      <div className="pt-4">
        <Card className="bg-gradient-to-br from-neutral-900 to-neutral-800 text-white rounded-[2rem] overflow-hidden border-0 shadow-2xl relative">
          ...
          <CardHeader>
            ...
            <div className="mt-8">
              <CardTitle className="text-2xl font-light tracking-widest uppercase">JOHN DOE</CardTitle>
              <CardDescription className="text-white/60 font-mono mt-1">**** **** **** 4029</CardDescription>
            </div>
          </CardHeader>
          <CardContent className="pt-4 pb-12 flex justify-between items-end relative z-10">
            <div className="space-y-1">
              <p className="text-[10px] uppercase tracking-widest opacity-40">Tier Status</p>
              <p className="text-sm font-bold text-amber-400">GOLD VIP ELITE</p>
            </div>
            <div className="text-right space-y-1">
               <p className="text-[10px] uppercase tracking-widest opacity-40">Expires</p>
